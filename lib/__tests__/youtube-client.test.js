...
describe("discovery search client", () => {
  ...
  const SECOND_CHANNEL_ID = "UCsecondchannel";

  function getFetchUrl(callIndex = 0) {
    return new URL(fetch.mock.calls[callIndex][0]);
  }

  function createDiscoveryChannelItem(channelId, overrides = {}) {
    return {
      id: channelId,
      snippet: {
        ...
        thumbnails: {
          high: { url: `https://yt3.ggpht.com/${channelId}` },
        },
      },
      statistics: {
  ...
  it("filters ineligible videos out of search results", async () => {
    ...
    expect(getFetchUrl(0).searchParams.get("order")).toBe("viewCount");
  });
  ...
  it("returns mapped channel search results", async () => {
    ...
    expect(getFetchUrl(0).searchParams.get("order")).toBe("videoCount");
  });

  it("preserves search.list order when channel details are returned out of order", async () => {
    const { fetchSearchChannels } = await import("@/lib/youtube-client");
    ...
    fetch
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          items: [
            { id: { channelId: DISCOVERY_CHANNEL_ID } },
            { id: { channelId: SECOND_CHANNEL_ID } },
          ],
        }),
      })
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          items: [
            createDiscoveryChannelItem(SECOND_CHANNEL_ID, {
              snippet: {
                title: "Second Channel",
                customUrl: "SecondChannel",
                description: "Second channel description.",
                thumbnails: {
                  high: { url: "https://yt3.ggpht.com/second" },
                },
              },
            }),
            createDiscoveryChannelItem(DISCOVERY_CHANNEL_ID, {
              snippet: {
                title: "Frontend Masters",
                customUrl: "FrontendMasters",
                description: "In-depth web development courses.",
    ...
    expect(result.channels.map((channel) => channel.youtubeChannelId)).toEqual([
      DISCOVERY_CHANNEL_ID,
      SECOND_CHANNEL_ID,
    ]);
    expect(result.channels[0].title).toBe("Frontend Masters");
    expect(result.channels[1].title).toBe("Second Channel");
  });
});
