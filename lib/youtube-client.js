...
async function youtubeSearch(type, query, pageToken, { order } = {}) {
  const params = {
    part: "snippet",
    type,
    q: query,
...
export async function fetchSearchChannels({ query, pageToken }) {
  ...
  const detailsById = new Map(channelDetails.map((item) => [item.id, item]));
  const channels = channelIds
    .map((id) => detailsById.get(id))
    .filter(Boolean)
    .map((item) => mapYouTubeDiscoveryChannelItem(item));
